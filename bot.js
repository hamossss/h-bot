...
client.on('message', function(message) {
    ...
    client.channels.get("467710323319898132").send({embed:iiMo});
    }
});
...
**-   [ *حكم ]**
 ...
 『:regional_indicator_h: 』『قريبا سوف نضيف المزيد و المزيد من الاوامر 』『:regional_indicator_h: 』
...
if (message.content.startsWith('*bcall')){
 if (message.author.id !== '386069723764490240') return message.reply('** هذا الأمر قفط لصاحب البوت و شكراًً **')
...
client.on('ready', function(){
    var ms = 30000 ;
    var setGame = [`*help | *inv `,` by:ོ,$!S7Q | ĦÂмØ♚#6947 !`];
    var i = -1;
    var j = 0;
...
client.on('message', message => {
  ...
  .setAuthor(message.author.username)
  .setColor("#9B59B6")
  .addField(" Done | تــــم" , "شيايك خاص يا حلو")
...
client.on ("guildMemberAdd", member => {
   ...
   var role = member.guild.roles.find ("name", "- USER H BOT");
   member.addRole (role);
